...
async def approve_role_request(request: RoleRequest, session: SessionDep) -> None:
    ...
    notification = NotificationCreate(body=settings.ROLE_REQUEST_APPROVED_MESSAGE, user_id=request.user.id, is_global=False)
...
async def reject_role_request(request: RoleRequest, session: SessionDep) -> None:
    await session.delete(request)
    await session.commit()
    notification = NotificationCreate(body=settings.ROLE_REQUEST_REJECTED_MESSAGE, user_id=request.user.id, is_global=False)
    await send_notification(notification, session, role=request.role.name)
