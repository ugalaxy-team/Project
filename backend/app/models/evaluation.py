...
evaluation_requirements = Table(
    ...
    Base.metadata,
    Column("requirement_evaluation_id", ForeignKey("requirement_evaluations.id", ondelete="CASCADE"), primary_key=True),
...
class SubmissionEvaluation(Base):
    ...
    submission_id: Mapped[int] = mapped_column(ForeignKey('submissions.team_id'), primary_key=True)
    ...
    evaluations: Mapped[list['RequirementEvaluation']] = relationship(back_populates='evaluation')
...
class RequirementEvaluation(Base):
    ...
    evaluation: Mapped['SubmissionEvaluation'] = relationship(back_populates='requirement_evaluations')
    score: Mapped[int]
