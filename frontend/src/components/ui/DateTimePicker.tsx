...
import { 
  ...
  startOfWeek, endOfWeek, isSameMonth, isSameDay, addDays, setHours, setMinutes, parseISO 
} from 'date-fns';
...
const DateTimePicker: React.FC<DateTimePickerProps> = ({ value, onChange, label }) => {
  ...
  return (
    <div className="relative pt-2">
      <span className="absolute top-0 left-3 bg-white px-1 text-[8px] font-black text-slate-400 uppercase z-10">{label}</span>
        ...
        >
          ...
          >
            <div className="p-4 min-w-[240px] bg-white relative z-10">
              <div className="flex items-center justify-between mb-4 px-1">
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-1 hover:bg-slate-50 rounded-lg text-slate-400 transition-colors"><ChevronLeft size={16}/></button>
                  <button type="button" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-1 hover:bg-slate-50 rounded-lg text-slate-400 transition-colors"><ChevronRight size={16}/></button>
                </div>
              </div>
            ...
            <div className="p-5 bg-slate-50 flex flex-col items-center justify-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 min-w-[140px]">
               <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Час</span>
               
               <div className="flex items-center gap-2">
                 <div className="flex flex-col items-center">
                   <button type="button" onClick={() => adjustTime('hours', 1)} className="p-1 text-slate-300 hover:text-[#6D72F1] transition-colors"><ChevronUp size={16}/></button>
                   ...
                   <button type="button" onClick={() => adjustTime('hours', -1)} className="p-1 text-slate-300 hover:text-[#6D72F1] transition-colors"><ChevronDown size={16}/></button>
                 </div>

                 <span className="text-slate-300 font-bold mb-1">:</span>

                 <div className="flex flex-col items-center">
                   <button type="button" onClick={() => adjustTime('minutes', 5)} className="p-1 text-slate-300 hover:text-[#6D72F1] transition-colors"><ChevronUp size={16}/></button>
                   <div className="w-10 h-12 bg-white rounded-xl flex items-center justify-center text-lg font-black text-[#6D72F1] shadow-sm border border-slate-100">
                     {format(dateValue, 'mm')}
                   </div>
                   <button type="button" onClick={() => adjustTime('minutes', -5)} className="p-1 text-slate-300 hover:text-[#6D72F1] transition-colors"><ChevronDown size={16}/></button>
                 </div>
               </div>
               ...
               <div className="flex gap-1">
                 {[9, 12, 18].map(h => (
                   <button
                    key={h}
                    type="button"
                    onClick={() => updateDateTime(setMinutes(setHours(new Date(dateValue), h), 0))}
