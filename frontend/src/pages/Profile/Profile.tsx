    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 font-sans text-[#111827]">
      <div className="max-w-[1000px] mx-auto space-y-6">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[16px] p-6 md:p-8 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              <div className="flex flex-col items-center sm:items-start gap-2">
                <h1 className="text-2xl font-bold">{user?.displayName || user?.full_name}</h1>
                <span className="bg-[#6366F1] text-white px-4 py-1.5 rounded-full text-sm font-medium">
                  Роль: {user?.roles?.length > 0 
                    ? user.roles.map((r: any) => r.display_name || r.name).join(", ") 
                    :   "Немає ролей"}
                </span>
              </div>
            </div>
          ...
          <div>
            <h3 className="text-xs font-bold text-[#6b7280] tracking-widest uppercase mb-4">СПОСОБИ ЗВ'ЯЗКУ</h3>
            <div className="flex flex-wrap gap-3">
              <ContactChip label="Email" value={user.email} />
              <ContactChip label="Telegram" value={user.telegram} colorClass="bg-[#eff6ff] border-[#bfdbfe] text-[#2563eb]" />
              <ContactChip label="GitHub" value={user.github} />
              <ContactChip label="Discord" value={user.discord} colorClass="bg-[#f5f3ff] border-[#ddd6fe] text-[#7c3aed]" />
            </div>
          </div>
        </div>
...
const ContactChip = ({ label, value, colorClass = "bg-[#f3f4f6] border-[#e5e7eb] text-[#111827]" }) => (
  <div className={`flex gap-1.5 px-4 py-2 rounded-lg border text-sm font-medium ${colorClass}`}>
...
const ListCard = ({ title, dotColor, items, isTeams = false }) => (
  <div className="bg-white border border-[#e5e7eb] rounded-[16px] p-6 shadow-sm">
    ...
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className="flex items-center justify-between p-4 bg-[#f9fafb] border border-[#e5e7eb] rounded-xl hover:bg-[#f3f4f6] transition-colors cursor-pointer">
          <div className="flex items-center gap-3">
            {isTeams && <div className="w-8 h-8 bg-[#e5e7eb] rounded-full flex items-center justify-center text-[#4b5563]">🤖</div>}
