...
describe("CreateTournamentModal", () => {
  ...
  const renderModal = () => {
    ...
    const view = render(
      <CreateTournamentModal isOpen={true} onClose={mockOnClose} onCreate={mockOnCreate} />
    );
    return { user, ...view };
  };

  const getForm = () => document.getElementById("create-tournament-form") as HTMLFormElement;
  const getBackdrop = () => document.querySelector(".bg-slate-900\\/40") as HTMLDivElement;
  ...
  describe("Rendering", () => {
    it("does not render anything when isOpen is false", () => {
      const { container } = render(
        <CreateTournamentModal isOpen={false} onClose={mockOnClose} onCreate={mockOnCreate} />
      );
    ...
    it("renders the modal dialog when isOpen is true", () => {
      ...
      expect(screen.getByText("СТВОРЕННЯ НОВОЇ ПОДІЇ У ВСЕСВІТІ")).toBeInTheDocument();
    });

    it("renders all form text inputs", () => {
      renderModal();
      expect(screen.getByPlaceholderText("Введіть круту назву...")).toBeInTheDocument();
      expect(screen.getByPlaceholderText("Про що цей турнір?")).toBeInTheDocument();
      expect(document.querySelector('input[name="max_teams"]')).toBeInTheDocument();
    });

    it("renders all datetime inputs", () => {
      renderModal();
      expect(document.querySelector('input[name="reg_start"]')).toBeInTheDocument();
      expect(document.querySelector('input[name="reg_end"]')).toBeInTheDocument();
  ...
  describe("Input Handling", () => {
    ...
    it("updates datetime inputs correctly via fireEvent", () => {
      renderModal();
      const regStartInput = document.querySelector('input[name="reg_start"]') as HTMLInputElement;
      
      fireEvent.change(regStartInput, { target: { name: "reg_start", value: "2026-06-01T12:00" } });
  ...
  describe("Form Submission", () => {
    const fillValidForm = async (user: any) => {
      ...
      const maxTeamsInput = document.querySelector('input[name="max_teams"]') as HTMLInputElement;
      const regStartInput = document.querySelector('input[name="reg_start"]') as HTMLInputElement;
      const regEndInput = document.querySelector('input[name="reg_end"]') as HTMLInputElement;
      ...
      fireEvent.change(regStartInput, { target: { name: "reg_start", value: "2026-05-01T10:00" } });
      fireEvent.change(regEndInput, { target: { name: "reg_end", value: "2026-05-15T10:00" } });
      fireEvent.change(startDateInput, { target: { name: "start_date", value: "2026-05-20T10:00" } });
    };
    ...
    it("resets form state after a successful submission", async () => {
      ...
      rerender(<CreateTournamentModal isOpen={true} onClose={mockOnClose} onCreate={mockOnCreate} />);
  ...
  describe("Loading and Error States", () => {
    ...
    it("logs error to console and stops loading if onCreate throws", async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith("Помилка при створенні турніру:", expect.any(Error));
      });

      expect(screen.getByText("СТВОРИТИ ТУРНІР")).toBeInTheDocument();
      const submitBtn = screen.getByText("СТВОРИТИ ТУРНІР") as HTMLButtonElement;
