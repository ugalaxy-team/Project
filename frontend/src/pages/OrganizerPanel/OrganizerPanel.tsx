...
const OrganizerPanel = () => {
  ...
  const [activeTab, setActiveTab] = useState<"tournaments" | "jury">(
    "tournaments",
  );
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="p-3 rounded-xl bg-indigo-50 border">
                  ...
                  <div className="text-sm font-semibold text-slate-700">
                    {formatDate(selectedTournament.reg_start)} —{" "}
                    {formatDate(selectedTournament.reg_end)}
                  </div>
                </div>
      ...
      {isJuryModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            ...
            onClick={() => setIsJuryModalOpen(false)}
          ></div>
          <div className="relative bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="bg-[#6366f1] p-8 text-white shrink-0">
              ...
              <h3 className="text-3xl font-black uppercase italic mb-1">
                ДОДАТИ ЖУРІ
              </h3>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p
                              className={`font-bold transition-all duration-300 ${isAdded ? "text-white" : "text-slate-800"}`}
                            >
                              {user.full_name}
                            </p>
                            <p
                              className={`text-xs transition-all duration-300 ${isAdded ? "text-white/70" : "text-slate-400"}`}
                            >
                              {user.email}
                            </p>
                          </div>
            ...
            <div className="p-6 border-t bg-white flex justify-between items-center">
              <span className="text-sm font-bold text-slate-500">
                Вибрано:{" "}
                <span className="text-[#6366f1]">{addedJurors.length}</span>
              </span>
              <button
                onClick={() => setIsJuryModalOpen(false)}
                className="bg-[#6366f1] text-white px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-widest"
              >
                Зберегти склад
              </button>
            </div>
