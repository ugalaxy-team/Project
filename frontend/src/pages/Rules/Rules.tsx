    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-24">
      <div className="relative bg-gradient-to-r from-[#6b73ff] to-[#4c51bf] pt-24 pb-48 px-6 overflow-hidden flex flex-col items-center">
        ...
        <p className="text-white/90 text-lg md:text-xl text-center max-w-2xl relative z-10 font-medium">
          Щоб перебування на платформі було комфортним та продуктивним для всіх,
          ми просимо дотримуватися кількох простих, але важливих правил.
        </p>
      </div>
      <div className="max-w-5xl mx-auto px-6 -mt-28 relative z-20 space-y-8">
        {rules.map((rule, index) => (
          ...
          >
            <div
              className={`absolute -right-6 -bottom-10 text-[180px] font-extrabold opacity-5 select-none transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-6 ${rule.color}`}
            >
              {rule.id}
            </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-8">
              <div
                className={`flex-shrink-0 w-20 h-20 rounded-2xl bg-slate-50 border-2 border-slate-100 flex items-center justify-center shadow-inner ${rule.color} transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}
              >
                {rule.icon}
              </div>
        ...
        <div className="bg-[#1e293b] rounded-[2rem] p-10 md:p-14 mt-16 shadow-2xl text-center relative overflow-hidden border border-slate-700">
          ...
          <p className="text-slate-400 mb-8 max-w-2xl mx-auto relative z-10 text-lg">
            Обирай свій перший турнір, збирай команду мрії та покажи, на що ти
            здатен.
          </p>
