...
const RoleRequestPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const info: RoleRequestOption[] = [
      ...
      {
        option_name: "Контакти",
        value: (formData.get("contact") as string) || "",
      },
      { option_name: "Вік", value: (formData.get("age") as string) || "" },
      {
        option_name: "Досвід",
        value: (formData.get("experience") as string) || "",
      },
      {
        option_name: "Причина",
        value: (formData.get("reason") as string) || "",
      },
  ...
  const inputClasses =
    "peer w-full px-[18px] pt-6 pb-2 border border-slate-200 rounded-xl bg-[#fafafa] text-sm text-gray-800 transition-all focus:outline-none focus:border-[#7b00ff] focus:bg-white focus:shadow-[0_0_0_3px_rgba(123,0,255,0.1)] placeholder-transparent";
  const labelClasses =
    "absolute left-[18px] top-1.5 text-[11px] font-medium text-[#7b00ff] pointer-events-none transition-all peer-placeholder-shown:top-[18px] peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400 peer-placeholder-shown:font-normal peer-focus:top-1.5 peer-focus:text-[11px] peer-focus:text-[#7b00ff] peer-focus:font-medium";
    ...
    <div className="flex flex-col md:flex-row min-h-screen font-sans bg-[#f7f8fc]">
      <div className="flex-1 bg-[#7b00ff] text-white flex flex-col items-center justify-center p-10">
        <div className="max-w-[480px] text-center md:text-left">
          ...
          <h2 className="text-[32px] m-0 font-extrabold text-center">
            UGalaxy
          </h2>
          <p className="text-sm tracking-[2px] mt-1 mb-10 opacity-90 text-center">
            STAR FOR LIFE
          </p>
          ...
          <p className="text-[15px] text-white/85 leading-[1.6] mb-6">
            Заповніть форму нижче, щоб подати заявку. Ми розглядаємо кожну
            заявку вручну.
            <br />
            <br />
            <b className="font-bold">Важливо:</b> відповідайте чесно та детально
            — це підвищує ваші шанси.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-10 px-5">
        <div className="bg-white w-full max-w-[420px] p-10 rounded-[24px] shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
          ...
          <p className="text-[13px] text-[#7b00ff] font-bold mb-4">
            * — обов'язкове поле
          </p>
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div className="relative w-full">
              <textarea
                ...
                className={`${inputClasses} min-h-[80px] resize-y`}
              ></textarea>
              <label htmlFor="plans" className={labelClasses}>
                Що ви плануєте робити на цій ролі? *
              </label>
            </div>
